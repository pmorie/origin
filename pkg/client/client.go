...
type DeploymentConfigInterface interface {
	...
	GetDeploymentConfig(name string) (deployapi.DeploymentConfig, error)
...
type DeploymentInterface interface {
	...
	GetDeployment(name string) (deployapi.Deployment, error)
...
func (c *Client) GetDeploymentConfig(name string) (result deployapi.DeploymentConfig, err error) {
	err = c.Get().Path("deploymentConfigs").Path(name).Do().Into(&result)
	return
}
...
func (c *Client) UpdateDeploymentConfig(deploymentConfig deployapi.DeploymentConfig) (result deployapi.DeploymentConfig, err error) {
	if deploymentConfig.ResourceVersion == 0 {
		err = fmt.Errorf("invalid update object, missing resource version: %v", deploymentConfig)
		return
	}
...
func (c *Client) DeleteDeploymentConfig(name string) error {
	return c.Delete().Path("deploymentConfigs").Path(name).Do().Error()
}
...
func (c *Client) GetDeployment(name string) (result deployapi.Deployment, err error) {
...
func (c *Client) UpdateDeployment(deployment deployapi.Deployment) (result deployapi.Deployment, err error) {
	if deployment.ResourceVersion == 0 {
		err = fmt.Errorf("invalid update object, missing resource version: %v", deployment)
		return
	}
